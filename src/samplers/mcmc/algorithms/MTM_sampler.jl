...
struct MultiTryMethod{
    ...
    MultiTryMethod(proposalspec::Q, weighting_scheme::WS, nproposals::Int, optimize_pt::Bool) where {
        Q<:ProposalDistSpec, W<:Real, WS<:MHWeightingScheme{W}} =
        new{Q,W,WS}(proposalspec, weighting_scheme, nproposals, optimize_pt)
end

export MultiTryMethod


MultiTryMethod(proposalspec::ProposalDistSpec = MvTDistProposalSpec()) =
    MultiTryMethod(proposalspec, MHMultiplicityWeights(), 10, true)

MultiTryMethod(weighting_scheme::MHWeightingScheme) =
    MultiTryMethod(MvTDistProposalSpec(), weighting_scheme, 10, true)

MultiTryMethod(nproposals::Int) =
    MultiTryMethod(MvTDistProposalSpec(), weighting_scheme, nproposals, true)


mcmc_compatible(::MultiTryMethod, ::AbstractProposalDist, ::NoParamBounds) = true

mcmc_compatible(::MultiTryMethod, proposaldist::AbstractProposalDist, bounds::HyperRectBounds) =
    issymmetric(proposaldist) || all(x -> x == hard_bounds, bounds.bt)


_sample_weight_type(::Type{MultiTryMethod{Q,W,WS}}) where {Q,W,WS} = W



mutable struct MTMIterator{
    SP<:MCMCSpec,
    R<:AbstractRNG,
...
function reset_rng_counters!(chain::MTMIterator)
    reset_rng_counters!(chain.rng, chain.info.id, chain.info.cycle, chain.stepno)
end


function (spec::MCMCSpec{<:MultiTryMethod})(
    chainid::Integer,
    exec_context::ExecContext
)
    ...
    MTMIterator(spec, info, Vector{P}(), exec_context)
end
...
function _available_samples_idxs(chain::MTMIterator)
...
function get_samples!(appendable, chain::MTMIterator, nonzero_weights::Bool)::typeof(appendable)
...
function get_sample_ids!(appendable, chain::MTMIterator, nonzero_weights::Bool)::typeof(appendable)
    if samples_available(chain)
        idxs = _available_samples_idxs(chain)
        samples = chain.samples
...
function next_cycle!(chain::MTMIterator)
...
function mcmc_step!(
    ...
    samples = chain.samples
    helpers_1 = chain.help_samples_1
    helpers_2 = chain.help_samples_2
